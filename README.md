...
`prev` и `final` являются частью имени плейсхолдера: `prev` означает коэффициент открытия, `final` - текущий коэффициент. Если имя и параметры плейсхолдера корректны, но данных нет, подставляется `0`. Некорректный плейсхолдер заменяется пустой строкой.
